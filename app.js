...
function updateDerivative() {
  try {
    ...
    document.getElementById("function-display").innerHTML = "Ошибка в функции";
    document.getElementById("derivative-display").innerHTML =
...
function bisectionMethod(f, epsilon) {
  ...
  if (iterations.length >= 10000)
    throw new Error("Превышено макс. итераций (10^4)");
...
function goldenSectionMethod(f, epsilon) {
  ...
  if (isNaN(f(x0_phi)) || f(x0_phi) === -Infinity || f(x0_phi) === +Infinity) {
    throw new Error("Ошибка вычисления f(x0_phi)");
  }
  let f1 = f(x0_phi);
  if (isNaN(f(x1_phi)) || f(x1_phi) === -Infinity || f(x1_phi) === +Infinity) {
...
function roundToPrecision(root, epsilon) {
  const decimalPlaces = Math.max(0, Math.abs(Math.floor(Math.log10(epsilon))));
